...
class DynamicPortConfigurationTest {
    ...
    void shouldEnsureThatServerWasStarted() {
        givenThat(get(urlEqualTo("/dynamic")).willReturn(aResponse()
                .withStatus(200)
                .withBody("dynamic configuration!")
        ));

        String serverUrl = buildServerUrl();
        ResponseEntity<String> response = restTemplate.getForEntity(serverUrl + "/dynamic", String.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isEqualTo("dynamic configuration!");
    }
